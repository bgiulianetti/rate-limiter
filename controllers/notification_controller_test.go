...
func TestNotificationController_SendNotification(t *testing.T) {
	testCases := []struct {
		...
		expectedCode                  int
		expectedResponse              string
		notificationServiceMockConfig func(*NotificationsServiceMock)
	}{
		{
			name:             "missing userID",
			userID:           "",
			...
			notificationServiceMockConfig: func(mock *NotificationsServiceMock) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			serviceMock := &NotificationsServiceMock{}
			tc.notificationServiceMockConfig(serviceMock)
